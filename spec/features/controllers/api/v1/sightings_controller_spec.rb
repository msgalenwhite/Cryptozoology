...
RSpec.describe Api::V1::SightingsController, type: :controller do
  ...
  describe "GET#index" do
    it "should return a list of sightings with the most recent sighting first" do
      ...
      expected_attributes = ["user_id", "cryptid_id", "location", "description", "pic_url", "rating", "identified"]
      ...
      expect(returned_json[0]["user_id"]).to eq(first_sighting.user_id)
      expect(returned_json[0]["cryptid_id"]).to eq(first_sighting.cryptid_id)
      ...
      expect(returned_json[1]["user_id"]).to eq(second_sighting.user_id)
      expect(returned_json[1]["cryptid_id"]).to eq(second_sighting.cryptid_id)
